...
It defines a 2D Concolutional layer that has a Gaussian prior on the parameters
"""
class GaussianConv2dReparameterization(nn.Module):
    ...
    def forward(self, X):
        W = self.W_mu + af.adapted_softplus(self.W_std) *\
        ...
        if self.bias:
            b_mu = self.b_mu.to(self.W_std.device)
            b_std = self.b_std.to(self.W_std.device)
            b = b_mu + af.adapted_softplus(b_std) *\
                torch.randn((self.out_channels), device=self.W_std.device)
        else:
