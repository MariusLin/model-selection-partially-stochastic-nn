...
from Partial_stochasticity.Layers.gaussian_reparam_conv import GaussianConv2dReparameterization
from Partial_stochasticity.Layers.conv import Conv2d

"""
This is an implementation of the PreResNet by He et al. 2016
Here the first layer has a Gaussian prior on it whereas the remaining layers are deterministic
"""
def conv3x3(in_planes, out_planes, stride=1,
            scaled_variance=True, device = "cpu"):
    "3x3 convolution with padding"
    return Conv2d(
        in_planes, out_planes, kernel_size=3, stride=stride,
        padding=1, bias=False, scaled_variance=scaled_variance, device = device)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 ...
                 prior_per="layer", scaled_variance=True, device = "cpu"):
...
class GaussianPreResNetReparameterization(nn.Module):
    def __init__(self, depth, num_classes=10, block_name='BasicBlock',
        ...
        self.conv1 = GaussianConv2dReparameterization(
            3, 16, W_std=W_std, b_std=b_std, kernel_size=3, padding=1, bias=False, 
            prior_per=prior_per, **self.prior_params)
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                Conv2d(
                    ...
                    stride=stride, bias=False, device = self.device, **self.prior_params),
            )
