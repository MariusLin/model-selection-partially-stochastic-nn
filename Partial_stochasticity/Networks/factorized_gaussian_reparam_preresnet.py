...
"""
This is an implementation of PreResNet by He et al. 2016
Here we impose a Gaussian prior on all parameters where we perform DWF on the standard deviations
"""
    ...
    def get_b_std_masks(self):
        ...
        if self.downsample is not None:
            for l in self.downsample:
                downsample_list = []
                if l.bias:
                    b_std_masks.append(l.get_b_std_mask())
...
class Bottleneck(nn.Module):
    ...
    def get_W_std_masks(self):
        ...
        if self.downsample is not None:
            for l in self.downsample:
                W_std_masks.append(l.get_W_std_mask())
        return W_std_masks
...
class FactorizedGaussianPreResNetReparameterization(nn.Module):
    ...
    def get_det_masks (self):
        # Go through all the layers and append the masks            
        layer1_list_weight = []
        layer1_list_bias = []
        for layer in self.layer1:
            layer1_list_weight.append(layer.get_W_std_masks())
            if self.bias:
                layer1_list_bias.append(layer.get_b_std_masks())
        layer2_list_weight = []
        layer2_list_bias = []
        for layer in self.layer2:
            layer2_list_weight.append(layer.get_W_std_masks())
            if self.bias:
                layer2_list_bias.append(layer.get_b_std_masks())
        layer3_list_weight = []
        layer3_list_bias = []
        for layer in self.layer3:
            layer3_list_weight.append(layer.get_W_std_masks())
            if self.bias:
                layer3_list_bias.append(layer.get_b_std_masks())
