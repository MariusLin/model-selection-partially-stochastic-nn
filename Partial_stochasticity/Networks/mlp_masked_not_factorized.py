...
class MLPMaskedNF(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims, activation_fn, weight_masks, bias_masks, D, 
        ...
        super(MLPMaskedNF, self).__init__()
        ...
        self.output_layer = MaskedLinear(hidden_dims[-1], output_dim, self.bias_masks[-1],
                self.weight_masks[-1], D = D, W_std = W_std_out, b_std = b_std_out, scaled_variance=scaled_variance, 
                prior_b_std=self.prior_b_std_list[-1], prior_W_std=self.prior_W_std_list[-1], device=self.device)
