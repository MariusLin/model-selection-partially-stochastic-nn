...
from Partial_stochasticity.Networks.bayes_net_masked import BayesNetMasked
...
class ClassificationNetMasked(BayesNetMasked):
    ...
    def _print_evaluations(self, x, y, train=True, num_pruned_det = 0, num_pruned_stoch = 0):
        ...
        if train:
            self.print_info("Samples # {:5d} : NLL = {:.5f} "
                "Acc = {:.4f} \n Pruned deterministic weights: {:d} Pruned stochastic weights: {:d}".format(self.num_samples, nll_, acc_, num_pruned_det, num_pruned_stoch))
        else:
            self.print_info("Validation: NLL = {:.5f} Acc = {:.4f} \n Pruned  deterministic weights: {:d} Pruned  stochastic weights: {:d}".format(
                nll_, acc_, num_pruned_det, num_pruned_stoch))
