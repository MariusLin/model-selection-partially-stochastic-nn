...
class RegressionNetMasked(BayesNetMasked):
    ...
    def _print_evaluations(self, x, y, train=True, num_pruned_det = 0):
        ...
        if train:
            self.print_info("Samples # {:5d} : NLL = {:11.4e} RMSE = {:.4e}  \n Pruned deterministic weights: {:d}".format(
                self.num_samples, total_nll, total_rmse, num_pruned_det))
        else:
            self.print_info("Validation: NLL = {:11.4e} RMSE = {:.4e}  \n Pruned deterministic weights: {:d}".format(
                total_nll, total_rmse, num_pruned_det))
