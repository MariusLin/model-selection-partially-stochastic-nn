...
os.chdir("..")
print ("Running fully stochastic regression with sparse BNN")
...
D = 3
mapper_batch_size = 128                                         # The factorization depth
...
def train(dataset):
    ...
    for split_id in range(n_splits):
        ...
        net = MLPMasked(input_dim, output_dim, [n_units] * n_hidden, activation_fn, weight_mask, bias_mask, 
                        W_std = W_std_out, b_std = b_std_out, device = device)
        net = net.to(device)
        ...
        saved_dir = os.path.join(out_dir, str(split_id))
        bayes_net = RegressionNet(net, likelihood, prior, saved_dir, n_gpu=n_gpu)
