...
os.chdir("..")
print ("Running CNN where the first layer is stochastic and the remaining ones are deterministic")

from Prior_optimization.gpr import GPR
from Prior_optimization import kernels, mean_functions
from Partial_stochasticity.Networks.gaussian_reparam_preresnet import GaussianPreResNetReparameterization
from Partial_stochasticity.Networks.preresnet import PreResNet
...
def setup(dataset):
    out_dir = f"./exp/vision/{dataset}/first_layer_stochastic"
...
def prior_optimization(dataset_name, train_test_splits, dataset):
    ...
    for split_id in range(n_splits):
        ...
        net_reparam = GaussianPreResNetReparameterization(depth=20, prior_per="parameter", device = device)
        net_reparam = net_reparam.to(device)
        mapper = MapperWasserstein(gp, net_reparam, rand_generator, out_dir=saved_dir,
