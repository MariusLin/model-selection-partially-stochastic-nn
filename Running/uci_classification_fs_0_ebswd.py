...
os.chdir("..")
print ("Running fully stochastic classififcation")
...
def setup(dataset):
    n_splits = 10
    n_units = 100
    out_dir = f"./exp/uci/{dataset}/fully_stochastic_ebswd"
...
def prior_optimization(dataset):
    ...
    for split_id in range(n_splits):
        ...
        input_dim, output_dim = int(X_train.shape[-1]), num_classes
...
def train(dataset):
    ...
    for split_id in range(n_splits):
        ...
        net = MLP(input_dim, num_classes, [n_units] * n_hidden, activation_fn, task = "classification")
        net = net.to(device)
