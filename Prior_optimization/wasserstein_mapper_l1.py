...
"""
This is a slight adaptation of the Wasserstein mapper from Tran et al. 2022
It defines a mapper to optimize the Wasserstein distance between the neural network and the GP
Additionally, it imposes an L_1 penalty on the standard deviations
"""
...
class MapperWassersteinL1(object):
    def __init__(self, gp, bnn, data_generator, out_dir, lambd,
                 input_dim=1, output_dim=1, n_data=256,
                 wasserstein_steps=(200, 200), wasserstein_lr=0.01, wasserstein_thres=0.01, 
                 logger=None, n_gpu=0, gpu_gp=False, lipschitz_constraint_type="gp", out_det = True):
        self.gp = gp
    ...
    def optimize(self, num_iters, n_samples=128, lr=1e-2,
                 save_ckpt_every=50, print_every=10, debug=False, eps = 1.19e-7):
        wdist_hist = []
        ...
        std_params = []
        for layer in self.bnn.layers + [self.bnn.output_layer]:
            layer_std = []
            for name, param in layer.named_parameters():
                if "std" in name:
                    layer_std.append(param.view(-1)) 
            if layer_std:
                layer_std_row = torch.cat(layer_std)
                std_params.append(layer_std_row)
        ...
        for it in range(1, num_iters+1):
            ...
            if std_params:
                reg_loss_vector = torch.tensor([torch.sum(torch.abs(std_params_per_layer)) \
                                    for std_params_per_layer in std_params]).to(self.device)
                regularization = torch.matmul(self.lambd, reg_loss_vector)
            else:
                regularization = 0
            loss = wdist + regularization
            loss.backward()
                    ...
                    if isinstance(layer, nn.Sequential):
                        for l in layer:
                            if not isinstance(l, nn.BatchNorm2d):
                                num_pruned += (torch.abs(l.W_std) < eps).sum().item()
                                num_pruned += (torch.abs(l.b_std) < eps).sum().item()
                    else:
                        num_pruned += (torch.abs(layer.W_std) < eps).sum().item()
                        num_pruned += (torch.abs(layer.b_std) < eps).sum().item()
                if not self.out_det:
                    num_pruned += (torch.abs(self.bnn.output_layer.W_std) < eps).sum().item()
                    num_pruned += (torch.abs(self.bnn.output_layer.b_std) < eps).sum().item()
            pruned_hist = np.append(pruned_hist, num_pruned)
            if (it % print_every == 0) or it == 1:
                self.print_info(">>> Iteration # {:3d}: "
                                "Wasserstein Dist {:.4f} Number of pruned weights {:3d}".format(
                                    it, float(wdist), num_pruned))
