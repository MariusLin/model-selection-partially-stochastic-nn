...
from Prior_optimization.likelihoods import Gaussian
from Prior_optimization.densities import multivariate_normal

from Prior_optimization.gp_model import GPModel
...
class GPR(GPModel):
    ...
    def __init__(self, X, Y, kern, mean_function=None, **kwargs):
        ...
        """
        likelihood = Gaussian(dtype=X.dtype)
    ...
    def compute_log_likelihood(self, X=None, Y=None):
        ...
        if Y.shape[1] > 1:
            ...
            for i in range(int(Y.shape[1])):
                results.append(multivariate_normal(
