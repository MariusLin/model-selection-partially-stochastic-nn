...
"""
It is an implementation of the optimisation mapper with L_1 regularization on the standard deviations
"""
class PriorOptimisationMapperL1():
    ...
    def ebswd(self, nnet_samples, gp_samples, output_dim, L=10, p=2, device="cpu"):
        ...
        for dim in range(output_dim):
            ...
            theta = theta / torch.norm(theta, dim=1, keepdim=True)  # normalize each direction
            ...
            wasserstein_distance = torch.sum(wasserstein_distance ** p, dim=0)  # sum over sorted entries
    ...
    def optimize(self, net, data_generator, n_data, num_iters, output_dim, lambd,
                n_samples=128, lr=1e-1, print_every=100, save_ckpt_every = 500, eps = 1.19e-7):
        ...
        for it in range(1, num_iters+1): 
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(std_params, 100.)
            ...
            if std_params:
                ...
                if not self.out_det:
                    num_pruned += (torch.abs(net.output_layer.W_std) < eps).sum().item()
                    num_pruned += (torch.abs(net.output_layer.b_std) < eps).sum().item()
            ...
            if ((it) % save_ckpt_every == 0) or (it == num_iters):
                ...
                if std_params:
                    ...
                    if self.out_det:
                        ...
                        checkpoint["output_layer_W_std"] = net.output_layer.W_std
                        checkpoint["output_layer_b_std"] = net.output_layer.b_std
