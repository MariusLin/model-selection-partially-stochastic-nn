...
import numpy as np
import torch
from Prior_optimization.parameter import PositiveParam
from Prior_optimization.quadrature import hermgauss
from Prior_optimization.densities import gaussian, bernoulli, exponential
...
class Likelihood(torch.nn.Module):
    ...
    def predict_density(self, Fmu, Fvar, Y):
        ...
        """
        gh_x, gh_w = hermgauss(self.num_gauss_hermite_points, dtype=Fmu.dtype)
...
class Gaussian(Likelihood):
    def __init__(self, dtype=torch.float32):
        ...
        self.variance = PositiveParam(torch.tensor([1.0], dtype=dtype), dtype=dtype)

    def logp(self, F, Y):
        return gaussian(F, Y, self.variance.get())
    ...
    def predict_density(self, Fmu, Fvar, Y):
        return gaussian(Fmu, Y, Fvar + self.variance.get())
...
class Exponential(Likelihood):
    ...
    def logp(self, F, Y):
        return exponential(self.invlink(F), Y)
