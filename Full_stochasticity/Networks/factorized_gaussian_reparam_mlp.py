...
"""
This is an implementation of a multi-layer perceptron with a Normal prior on the parameters.
The standard deviations are factorized using DWF
"""
class FactorizedGaussianMLPReparameterization(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims, activation_fn, D,
        ...
        self.output_layer = FactorizedGaussianLinearReparameterization(
                            hidden_dims[-1], output_dim, self.D, W_std, b_std,
                            scaled_variance=scaled_variance, prior_per="parameter", device=self.device)
