...
from Full_stochasticity.Layers.masked_conv import MaskedConv2d
from Full_stochasticity.Layers.masked_linear import MaskedLinear

"""
It is an implementation of the PreResNet of He et al. 2016
It takes two additional parameters: the deterministic masks.
...
"""
def conv3x3(in_planes, out_planes, stride=1, W_det_mask = None, b_det_mask = None, 
            bias = False, scaled_variance=True, device = "cpu"):
    "3x3 convolution with padding"
    return MaskedConv2d(in_planes, out_planes, kernel_size=3, stride=stride, W_det_mask= W_det_mask, 
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, W_det_masks = None, b_det_masks = None,
                 norm_layer='batchnorm', scaled_variance=True, bias = False, device = "cpu"):
        super(BasicBlock, self).__init__()
        self.norm1 = init_norm_layer(inplanes, norm_layer)
        self.relu = nn.ReLU(inplace=True)
        if b_det_masks:
            self.conv1 = conv3x3(inplanes, planes, stride, b_det_mask=b_det_masks[0], W_det_mask=W_det_masks[0],
                bias = bias, scaled_variance=scaled_variance, device = device)
        else:
            self.conv1 = conv3x3(inplanes, planes, stride, W_det_mask=W_det_masks[0],
                bias = bias, scaled_variance=scaled_variance, device = device)
        self.norm2 = init_norm_layer(inplanes, norm_layer)
        if b_det_masks:
            self.conv2 = conv3x3(planes, planes, W_det_mask=W_det_masks[1], b_det_mask=b_det_masks[1], bias = bias,
                                scaled_variance=scaled_variance, device = device)
        else:
            self.conv2 = conv3x3(planes, planes, W_det_mask=W_det_masks[1], bias = bias,
                                scaled_variance=scaled_variance, device = device)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, W_det_masks = None, b_det_masks = None,
        ...
        self.norm1 = init_norm_layer(inplanes, norm_layer)
        if b_det_masks:
            self.conv1 = MaskedConv2d(inplanes, planes, kernel_size=1, bias=bias, b_det_mask= b_det_masks[0],
                W_det_mask= W_det_masks[0], scaled_variance=scaled_variance, device = device)
        else:
            self.conv1 = MaskedConv2d(inplanes, planes, kernel_size=1, bias=bias, W_det_mask=W_det_masks[0],
                            scaled_variance=scaled_variance, device = device)
        self.norm2 = init_norm_layer(inplanes, norm_layer)
        if b_det_masks:
        ...
        if b_det_masks:
            self.conv3 = MaskedConv2d(planes, planes * 4, kernel_size=1, bias=bias, b_det_mask= b_det_masks[2],
            W_det_mask=W_det_masks[2], scaled_variance=scaled_variance, device = device)
        else:
            self.conv3 = MaskedConv2d(planes, planes * 4, kernel_size=1, bias=bias,
            W_det_mask=W_det_masks[2], scaled_variance=scaled_variance, device = device)    
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class MaskedPreResNet(nn.Module):
    def __init__(self, depth, weight_masks, bias_masks, bias_mask_out, num_classes=10, block_name='BasicBlock',
                 norm_layer='batchnorm', scaled_variance=True, device = "cpu"):
        super(MaskedPreResNet, self).__init__()
        ...
        self.inplanes = 16
        if bias_masks:
            self.conv1 = MaskedConv2d(3, 16, kernel_size=3, padding=1, b_det_mask=bias_masks[0][0],
               W_det_mask=weight_masks[0][0], bias=False, scaled_variance=scaled_variance, device = self.device)
        else:
            self.conv1 = MaskedConv2d(3, 16, kernel_size=3, padding=1, W_det_mask=weight_masks[0][0],
                    bias=False, scaled_variance=scaled_variance, device = self.device)
        if bias_masks:
            self.layer1 = self._make_layer(block, 16, n, weight_masks[1], bias_masks[1])
            self.layer2 = self._make_layer(block, 32, n, weight_masks[2], bias_masks[2], stride=2)
            self.layer3 = self._make_layer(block, 64, n, weight_masks[3], bias_masks[3], stride=2)
        else:
            self.layer1 = self._make_layer(block, 16, n, weight_masks[1])
            self.layer2 = self._make_layer(block, 32, n, weight_masks[2], stride=2)
            self.layer3 = self._make_layer(block, 64, n, weight_masks[3], stride=2)
        self.norm = init_norm_layer(64 * block.expansion, norm_layer)
        self.relu = nn.ReLU(inplace=True)
        self.avgpool = nn.AvgPool2d(8)
        self.fc = MaskedLinear(64 * block.expansion, num_classes, b_det_mask=bias_mask_out[0],
                    W_det_mask = weight_masks[-1][0], scaled_variance=scaled_variance,
                    device = self.device)
    ...
    def _make_layer(self, block, planes, blocks, W_det_masks, b_det_masks = None, stride=1):
        downsample = None
        ...
        layers = []
        if b_det_masks:
            layers.append(block(self.inplanes, planes, stride, downsample, scaled_variance=self.scaled_variance, 
                                W_det_masks = W_det_masks[0], b_det_masks = b_det_masks[0], device = self.device))
            self.inplanes = planes * block.expansion
            for i in range(1, blocks):
                layers.append(block(self.inplanes, planes, scaled_variance=self.scaled_variance, 
