...
"""
It is a masked version of a multi-layer perceptron
It takes two additional parameters: The masks for the layers.
The masks are of the following form: [mask of the input layer, ..., mask of the output layer]
"""
...
class MLPMasked(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims, activation_fn, weight_masks, bias_masks,
        ...
        self.layers = nn.ModuleList([MaskedLinear(
            input_dim, hidden_dims[0], self.bias_masks[0], self.weight_masks[0], W_std = W_std, b_std = b_std, 
        ...
        for i in range(1, len(hidden_dims)):
            self.layers.add_module(
                ...
                self.weight_masks[i], W_std = W_std, b_std = b_std, scaled_variance=scaled_variance, 
                device=self.device))
            self.norm_layers.add_module(
                "norm_{}".format(i), init_norm_layer(hidden_dims[i],
                                                     self.norm_layer))
        self.output_layer = MaskedLinear(hidden_dims[-1], output_dim, self.bias_masks[-1],
                self.weight_masks[-1], W_std = W_std, b_std = b_std, scaled_variance=scaled_variance, 
                device=self.device)
