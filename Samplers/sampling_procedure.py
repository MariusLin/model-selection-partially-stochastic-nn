...
class Mapper_Sampler:
    ...
    def sample_one_chain(self, net, dataloader, num_datapoints, X = None, y = None,
                         ...
                         print_every_n_samples = 5,  device = "cpu"):
        ...
        for step, (x_batch, y_batch) in batch_generator:
            ...
            loss = self._neg_log_joint(net, fx_batch, y_batch, num_datapoints)
                ...
                if n_samples > n_discarded:
                    ...
                    if self.num_samples % print_every_n_samples == 0:
                        ...
                        if (X is not None) and (y is not None):
                            self._print_evaluations(net, X, y, self.num_samples, self.sampled_weights, True)
                        else:
                            self._print_evaluations(net, x_batch, y_batch, self.num_samples, self.sampled_weights, True)
                        net.train()
...
class MapperSamplerJoint:
    ...
    def sample_multi_chains(self, net, data_loader, num_datapoints, X= None, y = None, num_chains=1, keep_every=200,
        ...
        for chain in range(num_chains):
            print("Chain: {}".format(chain+1))
            net.reset_parameters()
            self.sample_one_chain(net, data_loader, num_datapoints, X = X, y = y, keep_every = keep_every, 
                                  num_burn_in_steps = num_burn_in_steps, n_discarded = n_discarded, 
                                  num_samples = num_samples, print_every_n_samples = print_every_n_samples)
    ...
    def sample_one_chain(self, net, dataloader, num_datapoints, X = None, y = None,
        ...
        for param in stoch_params:
            param.requires_grad_(True)
        for param in det_params:
            param.requires_grad_(True)
        if self.adapted:
            sampler = AdaptiveSGHMC(stoch_params)
        else:
            sampler = SGHMC(stoch_params)
        det_optimizer = torch.optim.Adam(det_params, lr=0.001)
        # Start sampling
        net.train()
        n_samples = 0 # used to discard first samples
        for step, (x_batch, y_batch) in batch_generator:
            ...
            # ---- Compute Losses ----
            # NLL: for SGHMC/stochastic params
            nll_loss = self._neg_log_joint(net, fx_batch, y_batch, num_datapoints)

            # MSE: for deterministic params
            mse_loss = torch.nn.functional.mse_loss(fx_batch, y_batch)

            # ---- Backward ----

            # Backpropagate NLL for stochastic params
            nll_loss.backward(retain_graph=True)

            # Save current gradients of stochastic parameters (SGHMC will use them)
            stoch_grads = [p.grad.clone() for p in stoch_params]

            # Zero out gradients so we can compute MSE for deterministic parameters
            for p in net.parameters():
                p.grad = None

            # Backpropagate MSE for deterministic params
            mse_loss.backward()

            # Clip all gradients
            torch.nn.utils.clip_grad_norm_(net.parameters(), 100.)

            # Restore gradients for stochastic parameters (SGHMC needs them)
            for p, g in zip(stoch_params, stoch_grads):
                p.grad = g

            # Perform SGHMC and Adam steps
            sampler.step()
            det_optimizer.step()
            self.step += 1
            # Save the sampled weight
                ...
                if n_samples > n_discarded:
                    ...
                    if self.num_samples % print_every_n_samples == 0:
                        ...
                        if (X is not None) and (y is not None):
                            ...
                            self._print_evaluations(net, x_batch, y_batch, self.num_samples, 
